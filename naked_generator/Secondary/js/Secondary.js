...
function serializeSecondarySetPedalsRange(data) {
    return Uint8Array.from([
        ...byteify.serializeInt8(data.sync_state),
        ...byteify.serializeInt8(data.pedal),
    ]);
}
function deserializeSecondarySetPedalsRange(bytes) {
    return {
...
function deserializeSecondaryAcceleratorPedalVal(bytes) {
    return {
        level: byteify.deserializeUint8(bytes.slice(0, 1)),
    }
}

function serializeSecondaryBrakePedalVal(data) {
    return Uint8Array.from([
        ...byteify.serializeUint8(data.level),
    ]);
}
